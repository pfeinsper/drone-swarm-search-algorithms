...
class MLPModel(TorchModelV2, nn.Module):
    def __init__(
        ...
        **kw,
        ...
        self.fc_cnn = nn.Linear(self.fc1_input_size, 512)
        self.norm_cnn = nn.LayerNorm(512)

        self.fc_scalar = nn.Linear(obs_space[0].shape[0], 512)
        self.norm_scalar = nn.LayerNorm(512)
        self.policy_fn = nn.Linear(512 * 2, num_outputs)
        self.value_fn = nn.Linear(512 * 2, 1)
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        model_out = self.fc_cnn(model_out)
        model_out = self.norm_cnn(model_out)
        
        scalar_input = input_[0].float()
        scalar_out = self.fc_scalar(scalar_input)
        scalar_out = self.norm_scalar(scalar_out)
